...
def fix_contactes_columns():
    with engine.connect() as conn:
        ...
        try:
            # 1. Convertir carrec a VARCHAR
            conn.execute(text("ALTER TABLE contactes ALTER COLUMN carrec TYPE VARCHAR(100)"))
            ...
            # 2. Convertir to_preferit a VARCHAR
            conn.execute(text("ALTER TABLE contactes ALTER COLUMN to_preferit TYPE VARCHAR(50)"))
            
            conn.commit()
            print("✅ Taula contactes reparada amb èxit!")
